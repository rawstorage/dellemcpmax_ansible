...
def main():
    ...
    validhostlist = dellemc.get_host_list(
        filters="num_of_host_groups=0&num_of_masking_views=0")

    # Check if Storage Group already exists

    if module.params['host_or_cluster'] in validhostlist:
        dellemc.delete_hostgroup(hostgroup_id=module.params['host_or_cluster'])
        changed = True

    else:
        module.fail_json(msg='Unable to delete host, verify that host '
                             'exists and is not participating in '
                             'a masking view relationship')
