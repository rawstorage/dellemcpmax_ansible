...
author:
  - "Julien Brusset (@jbrt)"
short_description: "Remove Child SG to an existing Storage Group"
version_added: "2.8"
description:
  - "Removes existing child storage group, both parent and child storage 
  groups must exist and parent storage group should not contain any volumes.
...
EXAMPLES = '''
---
- name: "Remove Storage From DB Cluster"
  connection: local
  hosts: localhost
  vars:
...
def main():
    ...
    if module.params['parent_sg'] and module.params['child_sg'] in sglist:
        ...
        if dellemc.get_storage_group(module.params['parent_sg'])["type"] == "Standalone" \
            ...
            dellemc.remove_child_sg_from_parent_sg(child_sg=module.params[
                'child_sg'], parent_sg=module.params['parent_sg'])
            changed = True
        elif dellemc.get_storage_group(module.params['parent_sg'])["type"] == "Parent":
            dellemc.remove_child_sg_from_parent_sg(child_sg=module.params[
                'child_sg'], parent_sg=module.params['parent_sg'])
            changed = True
    else:
        module.fail_json(msg='Both Parent and Child SG must exist for module '
                             'to run successfully', changed=False)
